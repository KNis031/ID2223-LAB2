...
* **Task centric** Lastly, we could specify the task of the problem even more. Making it a domain specific task, say medicine or biology speech recognition we could for instance weigh sentences (data point) based on their relevance for the domain. Changing the task is a major step though and there are likely more that should/could be changed in that case.
